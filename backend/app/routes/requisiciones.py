...
def create_requisicion(payload: schemas.RequisicionCreate, db: Session = Depends(get_db)):
    ...
    requisicion = models.Requisicion(**payload.dict(exclude={"items", "comentarios"}))
    db.add(requisicion)
    db.flush()
...
def update_requisicion(
    requisicion_id: str, payload: schemas.RequisicionUpdate, db: Session = Depends(get_db)
):
    requisicion = (
    ...
    data = payload.dict(exclude_unset=True, exclude={"items", "comentarios"})

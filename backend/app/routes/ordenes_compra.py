...
def list_ordenes_compra(db: Session = Depends(get_db)):
    return db.query(models.OrdenCompra).order_by(models.OrdenCompra.fecha_creacion.desc()).all()
...
def create_orden_compra(payload: schemas.OrdenCompraCreate, db: Session = Depends(get_db)):
    ...
    orden = models.OrdenCompra(**payload.dict(exclude={"items"}))
    db.add(orden)
    db.flush()
...
def update_orden_compra(
    orden_id: str, payload: schemas.OrdenCompraUpdate, db: Session = Depends(get_db)
):
    orden = (
    ...
    data = payload.dict(exclude_unset=True, exclude={"items"})

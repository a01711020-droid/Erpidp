...
import {
  ...
  MaterialRequisition,
} from "@/app/components/MaterialRequisitionForm";
import {
  Plus,
  MessageSquare,
  Eye,
...
const INTERNAL_RESIDENTS_TEMPLATE: ResidentUser[] = [
  {
...
const INTERNAL_REQUISITIONS_TEMPLATE: MaterialRequisition[] = [
  {
    id: "1",
...
interface MaterialRequisitionsFullProps {
  residentsData?: ResidentUser[];
  requisitionsData?: MaterialRequisition[];
}

export default function MaterialRequisitionsFull({ residentsData = [], requisitionsData = [] }: MaterialRequisitionsFullProps) {
  ...
  const [requisitions, setRequisitions] = useState<MaterialRequisition[]>(requisitionsData);
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    const resident = residentsData.find((r) => r.name === selectedResidentName);

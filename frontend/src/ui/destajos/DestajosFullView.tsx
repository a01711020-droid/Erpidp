...
interface Props {
  ...
  destajistas?: DestajistaDTO[];
}

// Types
interface DestajistaDTO {
  id?: string;
  nombre: string;
  iniciales?: string;
  inicial?: string;
...
const obrasResumen: unknown[] = [];

export default function Destajos({
  initialState = "data",
  onBack,
  onManageDestajistas,
  destajistas: destajistasProp = [],
}: Props) {
  const [viewState, setViewState] = useState<ViewState>(initialState);
  const destajistas: Destajista[] = (destajistasProp ?? []).map((d, index) => ({
    id: d.id ? String(d.id) : String(index + 1),
    nombre: d.nombre,
    iniciales: d.iniciales ?? d.inicial ?? "",

...
// Destajistas - convertir de spec a formato local
const destajistas: Destajista[] = [].map((d, index) => ({
  id: String(index + 1),
  nombre: d.nombre,
  iniciales: d.inicial,
  color: d.color,
...
// Datos - Resumen de Obras para ResumenDestajos (importado desde spec)
const obrasResumen = [];
...
export default function Destajos({
  ...
  const validateCell = (key: string, value: string) => {
    const isValid = !value || destajistas.some((d) => d.iniciales === value);
    if (!isValid) {
      alert(`Error: "${value}" no es un destajista válido.\n\nDestajistas disponibles:\n${destajistas.map(d => `${d.iniciales} - ${d.nombre}`).join('\n')}`);
  ...
  const handleAddNota = () => {
    ...
    const destajista = destajistas.find((d) => d.iniciales === notaForm.iniciales);
    if (!destajista) {
  ...
  const handleExportPDF = () => {
    ...
    try {
      generateDestajosPDF(selectedObra, capturaData, historicos, destajistas);
    } catch (error) {
  ...
  const calculateCurrentWeekPayments = () => {
    ...
    Object.entries(capturaData).forEach(([key, iniciales]) => {
      ...
      if (!payments[iniciales]) {
        const dest = destajistas.find((d) => d.iniciales === iniciales);
  ...
  // ESTADO: EMPTY
  if (viewState === "empty") {
    return <DestajosStateEmpty onCreateWork={handleCreateWork} />;
  }
  ...
  if (selectedDestajista) {
    ...
    const destajista = destajistas.find((d) => d.iniciales === selectedDestajista);
                      ...
                      return (
                        <tr key={concepto.id} className={isOdd ? "bg-[#e8e8e8]" : "bg-white"}>
                          ...
                          {selectedObra.lotes.map((lote, idx) => {
                            ...
                            const isValidDestajista = destajistas.some((d) => d.iniciales === value);
                            ...
                            {(() => {
                              ...
                              allCurrentWeekDetails.forEach((item, idx) => {
                                ...
                                if (item.iniciales !== currentDestajista && currentDestajista !== "") {
                                  const destajista = destajistas.find((d) => d.iniciales === currentDestajista);
                                  rows.push(
                                ...
                                const destajista = destajistas.find((d) => d.iniciales === item.iniciales);
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-blue-900 mb-2 flex items-center gap-2">
                  ...
                  <CatalogoImportExport
                    catalogoConceptos={obras[0]?.catalogoConceptos || []}
                    nombreObra={obras[0]?.nombre || ""}
                    codigoObra={obras[0]?.codigo || ""}
                  />
                </h3>

...
export default function SupplierManagement({ onBack }: SupplierManagementProps) {
  ...
  useEffect(() => {
    if (proveedoresQuery.data.length) {
      setSuppliers(
        proveedoresQuery.data.map((proveedor) => ({
          id: proveedor.id,
          proveedor: proveedor.nombreComercial || proveedor.razonSocial,
          razonSocial: proveedor.razonSocial,
          rfc: proveedor.rfc,
          direccion: proveedor.direccion || "",
          vendedor: proveedor.contactoPrincipal || "",
  ...
  const handleDelete = async (id: string) => {
    if (confirm("¿Estás seguro de eliminar este proveedor?")) {
      await deleteProveedor.mutate({ id });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingSupplier) {
      await updateProveedor.mutate({
        id: editingSupplier.id,
        data: {
          razonSocial: formData.razonSocial,
          nombreComercial: formData.proveedor,
          rfc: formData.rfc,
          direccion: formData.direccion,
          contactoPrincipal: formData.vendedor,
          telefono: formData.telefono,
          email: formData.correo,
        },
      });
    } else {
      await createProveedor.mutate({
        razonSocial: formData.razonSocial,
        nombreComercial: formData.proveedor,
        rfc: formData.rfc,
        direccion: formData.direccion,

...
export default function MaterialRequisitions({ initialState = "data" }: MaterialRequisitionsProps = {}) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [obrasResponse, requisicionesResponse] = await Promise.all([
          dataProvider.listObras(),
          dataProvider.listRequisiciones(),
        ]);

        const obraIndex = new Map(
          obrasResponse.data.map((obra) => [obra.id, obra])
        );
        ...
        const mappedRequisitions = requisicionesResponse.data.map((requisicion) => {
          ...
          return {
            ...
            urgency:
              requisicion.urgencia === "urgente"
                ? "Urgente"
                : requisicion.urgencia === "muy_urgente"
                ? "Urgente"
                : "Normal",
        ...
        const mappedResidents = obrasResponse.data.map((obra) => ({
          name: obra.residente,

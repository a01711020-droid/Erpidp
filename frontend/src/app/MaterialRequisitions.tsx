...
import { useObras, useRequisiciones } from "@/core/hooks/useEntities";
...
export default function MaterialRequisitions({ initialState = "data" }: MaterialRequisitionsProps = {}) {
  ...
  useEffect(() => {
    if (obrasQuery.loading || requisicionesQuery.loading) {
      setViewState("loading");
      return;
    }

    if (obrasQuery.error || requisicionesQuery.error) {
      setViewState("error");
      return;
    }

    const obraIndex = new Map(
      obrasQuery.data.map((obra) => [obra.id, obra])
    );
    ...
    const mappedResidents = obrasQuery.data.map((obra) => ({
      name: obra.residente,
      initials: obra.residente
        .split(" ")
        .filter(Boolean)
    ...
    setViewState(mappedRequisitions.length ? "data" : "empty");
  }, [
    obrasQuery.data,
    obrasQuery.loading,
    obrasQuery.error,
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!password.trim()) {
      setAuthError("Ingrese la contraseña");
      return;
    }
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      {showForm && currentResident && (
        <MaterialRequisitionForm
          ...
          works={residents.map((resident) => ({
            code: resident.workCode,
            name: resident.workName,

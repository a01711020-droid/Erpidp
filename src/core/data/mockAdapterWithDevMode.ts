/**
 * MOCK ADAPTER CON DEV MODE
 * 
 * Wrapper del mockAdapter que intercepta las llamadas
 * y retorna datos según el modo de desarrollo activo
 */
...
// ============================================================================
// CONFIGURACIÓN GLOBAL (accesible desde el contexto)
// ============================================================================

let currentDevMode: DevMode = 'withData';
let currentNetworkDelay = true;

export function setDevModeConfig(mode: DevMode, networkDelay: boolean) {
  currentDevMode = mode;
  currentNetworkDelay = networkDelay;
}

export function getDevModeConfig() {
  return { mode: currentDevMode, networkDelay: currentNetworkDelay };
}
...
/**
 * Simular delay de red según configuración
 */
async function simulateDelay(): Promise<void> {
  if (!currentNetworkDelay) return;
  const delay = Math.random() * (MAX_DELAY - MIN_DELAY) + MIN_DELAY;
  await new Promise((resolve) => setTimeout(resolve, delay));
}

/**
 * Interceptar respuesta según el modo activo
 */
async function interceptResponse<T>(
  fetcher: () => Promise<DataResponse<T> | ListResponse<T>>
): Promise<DataResponse<T> | ListResponse<T>> {
  // Modo loading: nunca resuelve
  if (currentDevMode === 'loading') {
    await new Promise(() => {}); // Promise que nunca se resuelve
  }

  // Modo error: retornar error
  if (currentDevMode === 'error') {
    await simulateDelay();
  ...
  // Simular delay si está activo
  await simulateDelay();

  // Modo empty o withData: ejecutar fetcher y usar base de datos correspondiente
  return fetcher();
}

/**
 * Obtener base de datos según el modo
 */
function getDatabase() {
  return currentDevMode === 'empty' ? emptyDatabase : mockDatabase;
}
...
export class MockDataAdapterWithDevMode implements IDataAdapter {
  // ==========================================================================
  // OBRAS
  // ==========================================================================

  async listObras(filters?: { estatus?: string }): Promise<ListResponse<any>> {
    return interceptResponse(async () => {
  ...
  async getObra(obraId: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
  ...
  async createObra(data: any): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const nuevaObra = {
  ...
  async updateObra(obraId: string, data: any): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
      const obra = db.obras.find((o) => o.obra_id === obraId);
  ...
  async getProveedor(proveedorId: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
  ...
  async createProveedor(data: any): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
  ...
  async updateProveedor(proveedorId: string, data: any): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
      const proveedor = db.proveedores.find((p) => p.proveedor_id === proveedorId);
  ...
  async getRequisicion(requisicionId: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
  ...
  async createRequisicion(data: any): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
  ...
  async getOrdenCompra(ocId: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
  ...
  async createOrdenCompra(data: any): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
      const obra = db.obras.find((o) => o.obra_id === data.obra_id);
      if (!obra) {
  ...
  async updateOrdenCompraEstatus(ocId: string, estatus: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
      const orden = db.ordenes_compra.find((o) => o.oc_id === ocId);
  ...
  async getPago(pagoId: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
  ...
  async createPago(data: any): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
  ...
  async getEntrega(entregaId: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
  ...
  async getMetricasObra(obraId: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
      const obra = db.obras.find((o) => o.obra_id === obraId);
  ...
  async getAllMetricasObras(): Promise<ListResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
      const metricas: any[] = [];
  ...
  async getResumenProveedorOC(proveedorId: string): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
      const proveedor = db.proveedores.find((p) => p.proveedor_id === proveedorId);
      ...
      const ordenesProveedor = db.ordenes_compra.filter(
        (o) =>
          o.proveedor_id === proveedorId &&
          (o.estatus === 'aprobada' || o.estatus === 'entregada')
      );
  ...
  async getEstadoRequisiciones(obraId?: string): Promise<ListResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
      const obras = obraId ? db.obras.filter((o) => o.obra_id === obraId) : db.obras;

      const estados = obras.map((obra) => {
  ...
  async getDashboardGlobal(): Promise<DataResponse<any>> {
    return interceptResponse(async () => {
      const db = getDatabase();
...
export const mockAdapterWithDevMode = new MockDataAdapterWithDevMode();
